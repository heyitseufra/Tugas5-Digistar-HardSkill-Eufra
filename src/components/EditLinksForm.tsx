...
const EditLinksForm: React.FC<EditLinksFormProps> = ({ links, onSave }) => {
  ...
  return (
    <div className="p-6 rounded-lg shadow-md bg-pink-50">
      <h3 className="mb-4 text-xl font-bold text-pink-800 text-center"> {/* Align center untuk judul */}
        Update Links
      </h3>
      <form>
        {Object.keys(tempLinks).map((key) => (
          <div className="mb-4" key={key}>
            <label className="block text-pink-700 capitalize">{key}</label>
            <input
              type="text"
              name={key}
              value={tempLinks[key as keyof Links]}
              onChange={handleLinksChange}
              className={`w-full px-3 py-2 border rounded ${errors[key as keyof Links]
                ? "border-red-500"
                : "border-gray-300"
                }`}
            />
        ...
        <button
          type="button"
          className="w-full px-4 py-2 text-white transition bg-pink-600 rounded hover:bg-pink-700"
          onClick={handleSave}
        >
          Save
        </button>
