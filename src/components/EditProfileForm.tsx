...
const EditProfileForm: React.FC<EditProfileFormProps> = ({  
  ...
  return (  
    <div className="p-6 rounded-lg shadow-md bg-pink-50">  
      <h3 className="mb-4 text-xl font-bold text-pink-800">Update Profile</h3>  
      <form>  
        {Object.keys(refs).map((key) => (  
          <div className="mb-4" key={key}>  
            <label className="block text-pink-700 capitalize">  
              {key.replace(/([A-Z])/g, " $1")}  
            </label>  
            <input  
        ...
        <div className="flex items-center mb-4">  
          <input  
            ...
            className="hidden"  
          />  
          <label htmlFor="terms" className="flex items-center text-pink-700">  
            {termsAgreed ? (  
